...
class TaxReturnsConnectorSpec
  ...
  "create tax return" should {
    ...
    "return success response" when {
      ...
      "valid request send" in {
        val ret = aTaxReturn()
        val pptReference = ret.id
        val bod = Json.toJsObject(ret).toString

        givenPostTaxReturnEndpointreturns(200,pptReference,bod)
        val res: Either[ServiceError, Unit] = await(connector.submit(ret))
  ...
  private def givenPostTaxReturnEndpointreturns(
                                                 status: Int,
                                                 pptReference: String,
                                                 body: String = ""): StubMapping =
    stubFor(WireMock.post(s"/returns-submission/$pptReference")
      .willReturn(
        aResponse()
        .withStatus(status)
        .withBody(body))
