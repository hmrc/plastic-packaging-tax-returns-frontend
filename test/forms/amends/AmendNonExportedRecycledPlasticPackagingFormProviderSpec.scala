...
class AmendNonExportedRecycledPlasticPackagingFormProviderSpec extends LongFieldBehaviours {
  ...
  ".value" - {
    ...
    behave like longFieldWithMinimum(form,
      ...
      expectedError = FormError(
        fieldName,
        "amendRecycledPlasticPackaging.error.outOfRange.low",
        Seq(minimum)
      )
    )
    ...
    behave like longFieldWithMaximum(form,
      ...
      expectedError = FormError(
        fieldName,
        "amendRecycledPlasticPackaging.error.outOfRange.high",
        Seq(maximum)
      )
    )
