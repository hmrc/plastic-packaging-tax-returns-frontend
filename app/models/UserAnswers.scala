...
case class UserAnswers(
  ...
  /** Overload of
    * [[models.UserAnswers#getOrFail(queries.Gettable, play.api.libs.json.Reads, scala.reflect.api.TypeTags.TypeTag)]]
    * @param path
  ...
  def set[A](question: Settable[A], value: A, cleanup: Boolean = true)(implicit writes: Writes[A]): Try[UserAnswers] = {
    ...
    updatedData.flatMap { d =>
      ...
      if (cleanup) { question.cleanup(Some(value), updatedAnswers) }
  ...
  def setAll[A](all: (String, A)*)(implicit writes: Writes[A]): UserAnswers =
    copy(data = data ++ JsObject(all.map { case (x, y) =>
      (x, Json.toJson(y))
    }))
