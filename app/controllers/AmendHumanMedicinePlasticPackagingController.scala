...
import views.html.AmendHumanMedicinePlasticPackagingView
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendHumanMedicinePlasticPackagingPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(AmendHumanMedicinePlasticPackagingPage, mode, updatedAnswers))
      )
  }
}
